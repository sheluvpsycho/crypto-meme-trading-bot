...
class AdvancedTradingBot:
    def __init__(self, exchange_name: str = "kraken", api_key: str = None, 
        ...
        # RSI Settings - Very aggressive for DOGE (extreme volatility)
        self.rsi_period = 14
        ...
        # Risk Management
        self.initial_position_size = 0.85
        self.dynamic_stop_loss_base = 3.5
        self.trailing_stop_percent = 5.0
    ...
    def calculate_volatility(self, df):
        ...
        return max(0.02, min(volatility, 0.10))
    ...
    def get_signal(self, df):
        ...
        if self.current_position is not None:
            ...
            # Take profit (very high for DOGE)
            if pnl_percent > 20:
                return 'SELL'
    ...
    def execute_buy(self, current_price: float) -> bool:
        ...
        if self.account_type == "paper":
            ...
            print(f"🟢 BUY: {amount:.2f} {self.symbol.split('/')[0]} @ ${current_price:.4f}")
        else:
            try:
    ...
    def execute_sell(self, current_price: float) -> bool:
        ...
        try:
            if self.account_type == "paper":
                ...
                message = f"🔴 <b>DOGE SELL SIGNAL</b> 🐕\n\n"
                message += f"Coin: {self.symbol}\n"
                message += f"Amount: {amount:.2f}\n"
                message += f"Sell Price: ${current_price:.4f}\n"
                message += f"Entry Price: ${self.entry_price:.4f}\n"
                message += f"<b>PnL: ${pnl:.2f} ({pnl_percent:.2f}%)</b>\n"
                message += f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                
                self.send_telegram_alert(message)
                print(f"🔴 SELL: {amount:.2f} {self.symbol.split('/')[0]} @ ${current_price:.4f}")
    ...
    def run_loop(self, interval_seconds: int = 60, max_iterations: Optional[int] = None):
        iteration = 0
        print(f"\n🐕 Starting ADVANCED DOGE trading loop ({self.symbol})...")
        print(f"   Account type: {self.account_type.upper()}")
        print(f"   Strategy: Multi-Indicator + Risk Management (EXTREME)")
        ...
        try:
            while True:
                ...
                try:
                    ...
                    status = f"RSI: {rsi:.1f} | Price: ${current_price:.4f} | Signal: {signal}"
    ...
    def print_summary(self):
        ...
        print("\n" + "="*60)
        print("🐕 ADVANCED DOGE BOT - TRADING SUMMARY")
        print("="*60)
