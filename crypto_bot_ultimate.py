...
class UltimateTradingBot:
    ...
    def execute_buy(self, current_price: float) -> bool:
        ...
        msg = f"🟢 <b>ULTIMATE BOT BUY!</b> 💎\n\n{self.symbol}\nAmount: {amount:.6f}\nPrice: ${current_price:.2f}"
        self.send_telegram(msg)
        print(f"🟢 BUY: {amount:.6f} @ ${current_price:.2f}")
        return True
    ...
    def execute_sell(self, current_price: float) -> bool:
        ...
        msg = f"🔴 <b>ULTIMATE BOT SELL!</b> 💎\n\n{self.symbol}\nSell: ${current_price:.2f}\n<b>PnL: ${pnl:.2f} ({pnl_pct:.2f}%)</b>"
        self.send_telegram(msg)
        print(f"🔴 SELL: {amount:.6f} @ ${current_price:.2f} | PnL: ${pnl:.2f} ({pnl_pct:.2f}%)")
    ...
    def run_loop(self, interval_seconds: int = 60):
        print(f"\n💎 ULTIMATE BOT ACTIVATED! {self.symbol}")
        ...
        try:
            while True:
                try:
                    ...
                    status = f"Trend: {trend} | RSI: {rsi:.1f} | Price: ${current_price:.2f} | Signal: {signal}"
