...
class CryptoTradingBot:
    def __init__(self, exchange_name: str = "kraken", api_key: str = None, 
                 api_secret: str = None, symbol: str = "PEPE/USDT", 
        ...
        # More aggressive for PEPE (high volatility)
        self.rsi_period = 14
        self.rsi_oversold = 25  # Lower = more aggressive
        self.rsi_overbought = 75  # Higher = more aggressive
        self.position_size_percent = 0.95
        self.stop_loss_percent = 3  # Higher stop loss for volatility
        self.take_profit_percent = 8  # Wait for bigger gains
    ...
    def execute_buy(self, current_price: float) -> bool:
        ...
        if self.account_type == "paper":
            ...
            print(f"🟢 BUY: {amount:.2f} {self.symbol.split('/')[0]} @ ${current_price:.6f}")
        else:
            try:
    ...
    def execute_sell(self, current_price: float) -> bool:
        ...
        try:
            if self.account_type == "paper":
                ...
                print(f"🔴 SELL: {amount:.2f} {self.symbol.split('/')[0]} @ ${current_price:.6f}")
    ...
    def run_loop(self, interval_seconds: int = 60, max_iterations: Optional[int] = None):
        iteration = 0
        print(f"\n🐸 Starting PEPE trading loop ({self.symbol})...")
        ...
        try:
            while True:
                ...
                try:
                    ...
                    status = f"RSI: {rsi:.1f} | Price: ${current_price:.6f} | Signal: {signal}"
    ...
    def print_summary(self):
        ...
        print("\n" + "="*50)
        print("🐸 PEPE BOT - TRADING SUMMARY")
        print("="*50)
